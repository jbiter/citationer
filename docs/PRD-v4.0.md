# Citationer — 文献题录分析工具 · 产品需求文档 v4.0

> **状态**: Phase 5 推进中 / Phase 6 预规划
> **作者**: Jason
> **日期**: 2026-07-25
...
| 渠道 | 状态 | 说明 |
|------|------|------|
| **PyPI (pip/pipx)** | ✅ 已发布 | `citationer` v5.0.0，CI 自动发布 |
...
### Phase 5: 高级功能与生态 — v5.x 路线图

> 已进入执行阶段，按下方版本节奏发布；具体时间可根据用户反馈调整。

| # | 工作项 | 优先级 | 目标版本 | 目标时间 | 预估工作量 | 说明 |
|---|--------|--------|----------|----------|-----------|------|
...
| P5-2 | **Web UI（`citationer serve`）** | 🟡 P1 | v5.1.0 | 2026-08 | 3–4 周 | Flask/FastAPI 本地 Web 界面 + 交互式图表仪表盘；优先覆盖 stats / network / compare |
| P5-3 | **插件系统** | 🟢 P2 | v5.2.0 | 2026-08 | 2–3 周 | 第三方贡献解析器；entry_points 发现 + 标准接口；首个示例插件：EndNote 解析器 |
| P5-4 | **PDF 全文分析** | 🟢 P2 | v5.3.0 | 2026-09 | 3–4 周 | PDF 解析、全文主题建模、引文上下文分析；仅本地处理 |
| P5-5 | **实时文献监控** | 🔵 P3 | v5.4.0 | 2026-09 | 2–3 周 | RSS/API 追踪新增文献 + 邮件/终端通知；可配置监控源 |
| P5-7 | **多语言国际化 (i18n)** | 🔵 P3 | v5.5.0 | 2026-10 | 2 周 | gettext 或手动方案，CLI 输出中英文双语；先覆盖核心命令 |
| P5-8 | **EndNote / Zotero 解析器** | 🔵 P3 | v5.6.0 | 2026-10 | 1 周 | EndNote XML + Zotero RDF/CSV；视插件系统进度可能提前作为内置解析器 |
| P5-10 | **数据库查询命令** (`citationer query`) | ✅ 已完成 | v4.7.0 | 2026-07 | 1 周 | SQLite 直查或 DSL 过滤（按年份/期刊/作者/关键词筛选） |

> **下阶段重点**：v5.1.0 P5-2 Web UI。

### Phase 6: 平台化与智能化 — v6.x 路线图

> 2026 年 Q4 起进入 Phase 6，目标是从单机 CLI 工具向可协作、可扩展的文献分析平台演进。

| # | 工作项 | 优先级 | 目标版本 | 目标时间 | 预估工作量 | 说明 |
|---|--------|--------|----------|----------|-----------|------|
| P6-1 | **协作功能** | 🟡 P1 | v6.1.0 | 2026-11 | 3–4 周 | 共享分析报告、团队文献库、多人标注 |
| P6-2 | **引文网络** | 🟡 P1 | v6.2.0 | 2026-12 | 3–4 周 | 前向引用追踪 + 引文级联分析 |
| P6-3 | **AI 深度分析** | 🟢 P2 | v6.3.0 | 2027-01 | 2–3 周 | 多轮对话式文献探索、假设生成 |
| P6-4 | **期刊推荐** | 🟢 P2 | v6.4.0 | 2027-02 | 2 周 | 基于手稿摘要推荐投稿目标期刊 |
| P6-5 | **学术社交图谱** | 🔵 P3 | v6.5.0 | 2027-03 | 2–3 周 | 研究者合作关系发现 + 影响力分析 |

> Phase 6 各版本时间为初步规划，将在 v5.x 执行过程中根据反馈和资源情况滚动调整。
...
> **决策 (v4.0)**：维持 PyPI 为主渠道，当前已发布 `citationer` v5.0.0。Phase 4 补齐 PyInstaller 二进制分发。Docker 镜像与 conda-forge 不再列入路线图。Homebrew 不做。
...
> 📝 **下一版迭代建议**:
> 1. P5-2 Web UI（`citationer serve`）— Phase 5 首要目标，提升非技术用户可用性
> 2. P5-3 插件系统 — 为第三方解析器提供入口，降低 EndNote/Zotero 等格式接入成本
> 3. P5-4 PDF 全文分析 — 扩展本地分析能力，支持全文主题建模
> 4. 持续收集 GitHub Issues 反馈，按实际优先级滚动调整 v5.x 排期
