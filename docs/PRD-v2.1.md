...
| 选项 | 评估 | 结论 |
|------|------|------|
| `plotext` | 纯 Python、braille 字符折线图、水平/垂直条形图、终端宽度自适应、中文支持 | ✅ 选用 |
| `termgraph` | 仅条形图、功能单一 | ❌ |
| `uniplot` | 仅折线图/散点图 | ❌ |
| `rich` | 仅基础进度条样式柱状图，无折线图 | ❌ |
| `matplotlib` | 需 GUI 后端或文件输出、无终端渲染 | ❌ |

`plotext` 使用 Unicode braille 字符（`⣀⡀⠤⠒⠉`）绘制折线，在终端中呈现连续的平滑曲线，效果接近 GUI 图。
