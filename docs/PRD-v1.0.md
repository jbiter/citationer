# Citationer — 文献题录分析工具 · 产品需求文档 v1.0

> **状态**: 已归档 · 已被 [PRD-v3.0.md](PRD-v3.0.md) 取代 · 当前版本 v4.8.0  
> **作者**: Jason  
> **日期**: 2026-07-02  
> **许可证**: MIT · 开源项目  
...
--output, -o       指定输出目录
--config, -c       指定配置文件路径
--format           输出格式 (terminal/json/csv/table)
--language         语言设置 (zh/en)
--no-color         禁用彩色输出
```
...
### Phase 2: 文本挖掘 + LLM + 网络分析 — 预计 4-6 周

- [ ] 中文分词 (jieba) + 英文 NLP (spaCy) — 同时支持
- [ ] 内置中英文学术语停用词表
- [ ] 关键词频次统计 + 词云
- [ ] LDA / NMF 主题建模
- [ ] **LLM 集成 (DeepSeek)**: 主题自动标注、文献集综述生成
- [ ] **LLM 集成**: 研究趋势自动识别、多维分类
- [ ] `ai` 命令组 (topics, summarize, trends, classify)
- [ ] 关键词共现网络
- [ ] 作者/机构合作网络
- [ ] HTML 交互式网络图导出 (plotly/pyecharts)
- [ ] `network` 命令组
- [ ] `text keywords` / `text topics` / `text preprocess`
